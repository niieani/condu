...
- [ ] features should be able to contribute CLI command functionality via `defineFeature` - implement the ability for features to define their own CLI commands, e.g. `condu custom-command`, etc. we'll need to figure out how to disambiguate if multiple features contribute the same command.
